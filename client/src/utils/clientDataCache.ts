...
class ClientDataCache {
  ...
  async set<T>(key: string, data: T, options: CacheOptions = {}): Promise<void> {
    ...
    if (opts.strategy !== CacheStrategy.MEMORY) {
      try {
        ...
        console.warn(`Failed to persist data for key: ${key}`, _error);
      }
    }
  ...
  async get<T>(key: string, options: CacheOptions = {}): Promise<T | undefined> {
    ...
    if (opts.strategy !== CacheStrategy.MEMORY) {
      try {
        ...
        console.warn(`Failed to retrieve data for key: ${key}`, _error);
      }
    }
  ...
  async invalidate(key: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`Failed to invalidate data for key: ${key}`, _error);
    }
  }
  ...
  async preload<T>(
    ...
    try {
      ...
      console.warn(`Failed to preload data for key: ${key}`, _error);
    }
  }
  ...
  private async cacheOnlyStrategy<T>(
    ...
    if (data === undefined && options.strategy !== CacheStrategy.MEMORY) {
      try {
        ...
        console.warn(`Failed to retrieve data for key: ${cacheKey}`, _error);
      }
    }
  ...
  private async cacheFirstStrategy<T>(
    ...
    try {
      ...
      console.warn(`Cache read failed for key: ${cacheKey}`, _error);
    }
    ...
    if (listeners) {
      listeners.forEach(listener => {
        try {
          ...
          console.error(`Error in cache listener for key: ${key}`, _error);
        }
      });
    }
  }
